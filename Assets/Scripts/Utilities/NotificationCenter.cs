...
public class NotificationCenter
{
	...
	/// </summary>
	private Dictionary<Notifications, SenderTable> _table = new Dictionary<Notifications, SenderTable>();
	...
	#region Public
	public void AddObserver(Handler handler, Notifications notificationName)
	{
		AddObserver(handler, notificationName, null);
	}

	public void AddObserver(Handler handler, Notifications notificationName, System.Object sender)
	{
		if (handler == null)
		{
	...
	public void RemoveObserver(Handler handler, Notifications notificationName)
	{
		RemoveObserver(handler, notificationName, null);
	}

	public void RemoveObserver(Handler handler, Notifications notificationName, System.Object sender)
	{
		if (handler == null)
		{
	...
	public void Clean()
	{
		Notifications[] notKeys = new Notifications[_table.Keys.Count];
		_table.Keys.CopyTo(notKeys, 0);

		for (int i = notKeys.Length - 1; i >= 0; --i)
		{
			Notifications notificationName = notKeys[i];
	...
	public void PostNotification(Notifications notificationName)
	{
		PostNotification(notificationName, null);
	}

	public void PostNotification(Notifications notificationName, System.Object sender)
	{
		PostNotification(notificationName, sender, null);
	}

	public void PostNotification(Notifications notificationName, System.Object sender, System.Object e)
	{
		...
		if (sender != null && subTable.ContainsKey(sender))
		{
			...
			{
				//Debug.Log(handlers[i] + " heard " + notificationName);
